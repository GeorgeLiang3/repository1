...
from HessianMC import run_chain_hessian, run_chain_update_Hessian
...
class MCMC:
    ...
    def run_chain(self, method='RMH'):
        ...
        if self.method == 'updateHessian':
            accepted, rejected = run_chain_update_Hessian(self.target_distribution.cov, self.num_results, self.burnin,
                                                          self.unnormalized_posterior_log_prob)
            accepted = np.array(accepted)
            rejected = np.array(rejected)
            return accepted, rejected
